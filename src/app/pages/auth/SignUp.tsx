...
import { Car, ArrowLeft, Globe } from "lucide-react";
...
export default function SignUp() {
  ...
  const [accountType, setAccountType] = useState<"user" | "driver">("user");
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const result = await api.signup({
        ...
        phone,
        password,
        role: accountType === "user" ? "USER" : "DRIVER"
      });
      if (result.token) {
        setAuthToken(result.token);
      }
      localStorage.setItem("auth_user", JSON.stringify(result.user));
      navigate(accountType === "user" ? "/user/home" : "/driver-registration");
    } catch (error) {
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <Tabs value={accountType} onValueChange={(value) => setAccountType(value as "user" | "driver")} className="w-full">
